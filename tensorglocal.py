...
data=t_prod(np.random.rand(m,r,k),np.random.rand(r,n,k))
...
for ep in eplison:
    ...
    sigma=2*np.power(L,2)*np.sqrt(2*T*np.log10(1/delta))/(ep)
    print 'sigma:',sigma
...
#            lamdex=np.where(lamsqrt==np.max(lamsqrt))
        ...
        for it in range(n):
            for jt in range(n):
                for kt in range(k):
                    W[it][jt][kt]=W[it][jt][kt]+ np.random.normal(0,np.power(sigma,2))
